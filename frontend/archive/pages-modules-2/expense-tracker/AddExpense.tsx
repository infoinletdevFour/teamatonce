    ...
    <SimpleLayout>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {apiError && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 mb-6">
            <div className="flex items-center">
              <Close className="h-5 w-5 text-red-500 mr-3" />
        ...
        <Card className="mb-8 rounded-2xl bg-gradient-to-r from-white via-white to-primary/5 dark:from-gray-800 dark:via-gray-800 dark:to-primary/10 border-l-4 border-primary">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2">
              <CloudUpload className="h-5 w-5" style={{ color: 'rgb(71, 189, 255)' }} />
              Receipt Upload
            </CardTitle>
            <CardDescription>
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {receipts.map((receipt) => (
                    <div
                      key={receipt.id}
                      className="relative group border rounded-xl overflow-hidden bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-all"
                    >
                      {/* Preview */}
                      {receipt.file.type.startsWith('image/') ? (
                        ...
                        <div className="aspect-square bg-gray-100 dark:bg-gray-700 flex flex-col items-center justify-center p-4">
                          <InsertDriveFile className="h-12 w-12 text-gray-400 mb-2" />
                      ...
                      {!receipt.file.type.startsWith('image/') && (
                        <Button
                          ...
                          size="icon"
                          onClick={() => removeReceipt(receipt.id)}
                          className="absolute top-2 right-2 h-8 w-8 bg-white/80 dark:bg-gray-800/80 hover:bg-red-50 dark:hover:bg-red-900/20"
                        >
        ...
        <Card className="rounded-2xl">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2">
              <Receipt className="h-5 w-5" style={{ color: 'rgb(71, 189, 255)' }} />
              Expense Details
            </CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="title" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <Receipt className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
                Expense Title
              </Label>
              <Input
                id="title"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="amount" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <AttachMoney className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
                  Amount ($)
                </Label>
                <Input
                  id="amount"
                  ...
                  className={`h-12 rounded-xl ${errors.amount ? 'border-red-500' : ''}`}
                />
                {errors.amount && (
              ...
              <div className="space-y-2">
                <Label htmlFor="date" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <Calendar className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
                  Date
                </Label>
                <Input
                  id="date"
                  ...
                  className={`h-12 rounded-xl ${errors.date ? 'border-red-500' : ''}`}
                />
                {errors.date && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="category" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <Category className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
                  Category {categories.length === 0 && !categoriesLoading && <span className="text-xs text-gray-500">(Optional - categories unavailable)</span>}
                </Label>
                <select
                  id="category"
                  ...
                  className={`flex h-12 w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${errors.category ? 'border-red-500' : ''}`}
              ...
              <div className="space-y-2">
                <Label htmlFor="payment-method" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <Payment className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
                  Payment Method
                </Label>
                <select
                  id="payment-method"
                  ...
                  className={`flex h-12 w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${errors.paymentMethod ? 'border-red-500' : ''}`}
                >
            ...
            <div className="space-y-2">
              <Label htmlFor="description" className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <FileText className="h-4 w-4" style={{ color: 'rgb(71, 189, 255)' }} />
              ...
              <textarea
                ...
                className="w-full min-h-[120px] p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent resize-none"
                rows={4}
              />
            </div>
