...
const CongratulationsOverlay: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      ...
      <div className="relative bg-card/95 backdrop-blur-xl rounded-3xl p-10 mx-4 max-w-lg w-full text-center shadow-2xl border border-border/50">
        {/* Trophy icon */}
        <div className="mb-8 flex justify-center">
          <div className="w-24 h-24 bg-gradient-to-br from-[#47bdff] to-[#47bdff]/80 rounded-full flex items-center justify-center shadow-xl animate-bounce relative">
        ...
        {/* XP earned */}
        <div className="bg-gradient-to-r from-[#47bdff]/5 via-[#47bdff]/10 to-[#47bdff]/5 rounded-2xl p-6 mb-8 border border-[#47bdff]/20 shadow-inner">
          <div className="flex items-center justify-center space-x-3 mb-3">
            <Star className="h-7 w-7 text-[#47bdff] animate-spin" />
            <span className="text-3xl font-bold text-foreground tracking-tight">
              +{xpEarned} XP
            </span>
            <Star className="h-7 w-7 text-[#47bdff] animate-spin" style={{ animationDirection: 'reverse' }} />
          </div>
          <p className="text-sm text-muted-foreground font-medium">Experience Points Earned</p>
        </div>
        ...
        <div className="mb-8">
          ...
          <p className="text-[#47bdff] font-semibold mt-2">
            ¡Sigue así!
          </p>
        </div>
        ...
        <Button 
          onClick={onContinue}
          className="w-full h-14 text-lg font-semibold bg-gradient-to-r from-[#47bdff] to-[#47bdff]/90 hover:from-[#47bdff]/90 hover:to-[#47bdff]/80 text-white border-none shadow-lg transform hover:scale-[1.02] transition-all duration-300 rounded-xl"
        >
          Continue Learning
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 20 }).map((_, i) => (
          <Star
            key={i}
            className="absolute h-4 w-4 text-[#47bdff] animate-bounce"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
...
const LessonPage: React.FC = () => {
  ...
  const handlePlayAudio = async (text: string) => {
    ...
    try {
      ...
      const ttsUrls = [
        `https://translate.google.com/translate_tts?ie=UTF-8&tl=es&client=tw-ob&q=${encodeURIComponent(cleanText)}`,
        `https://api.voicerss.org/?key=demo&hl=es-es&src=${encodeURIComponent(cleanText)}`,
      ];
  ...
  if (!lessonData) {
      ...
      return (
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2">No Exercises Found</h2>
            <p className="text-muted-foreground mb-6">This unit doesn't have any exercises available yet.</p>
            <div className="space-x-4">
              <Button onClick={() => {
                if (currentLessonId) {
                  navigate(`/language-learner/learning?lesson_id=${currentLessonId}`);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="text-muted-foreground text-6xl mb-4">📚</div>
          <h2 className="text-2xl font-bold text-foreground mb-2">No Lesson Selected</h2>
          <p className="text-muted-foreground mb-6">Please select a lesson to begin learning.</p>
          <Button onClick={() => {
            if (currentLessonId) {
              navigate(`/language-learner/learning?lesson_id=${currentLessonId}`);
            } else {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-pink-50 via-blue-50 to-white dark:from-gray-900 dark:via-gray-800 dark:to-background overflow-x-hidden">
      ...
      {/* Lesson Progress Bar */}
      <div className="w-full bg-white/95 dark:bg-card/95 backdrop-blur-lg border-b border-border/30 px-4 py-5">
          ...
          <div className="flex items-center space-x-8">
            <div className="flex items-center space-x-2">
              <Button
                ...
                size="sm"
                onClick={handleExit}
                className="text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-full p-3 transition-all duration-200"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {specificExercise && (
          <div className="mb-6 p-4 bg-white/90 dark:bg-gray-900/60 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">{specificExercise.title}</h2>
                <p className="text-gray-600 dark:text-gray-400">Exercise {specificExercise.order_index} • {specificExercise.type.replace('_', ' ')}</p>
                <div className="flex items-center space-x-4 mt-2">
                  <Badge variant="outline" className="capitalize">
                    Difficulty {(specificExercise as any).difficulty_level}
                  </Badge>
                  <Badge variant={specificExercise.user_progress.status === 'completed' ? 'default' : 'outline'}>
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-[#47bdff]">
        ...
        {unitExercises && !specificExercise && (
          <div className="mb-6 p-4 bg-white/90 dark:bg-gray-900/60 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">{unitExercises.unit_info.title}</h2>
                <p className="text-gray-600 dark:text-gray-400">Unit Exercises • {(unitExercises as any).data.length} exercises total</p>
                <div className="flex items-center space-x-4 mt-2">
